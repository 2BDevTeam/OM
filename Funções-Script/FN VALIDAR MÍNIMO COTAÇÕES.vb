...
Dim incrementa As Boolean = False

If objpara.incrementa IsNot Nothing Then
    incrementa = CBool(objpara.incrementa)
End If
...
' 2. Obter procedimento de cotação (qtd mínima)
...
Dim qtdMin As Decimal = CDec(getProcResult.data("qtd"))
...
' 5. Verificar se atingiu o mínimo
Dim abaixoMinimo As Boolean = (qtdCot < qtdMin)

If abaixoMinimo Then
    Return New With {
        .cod = "0000",
        .codDesc = "Sucesso",
        .message = "Número de cotações (" & qtdCot & ") ainda não atinge o mínimo exigido (" & qtdMin & ") para o valor total da requisição.",
        .data = New With {.atingiuMinimo = False, .abaixoMinimo = True, .qtdCot = qtdCot, .qtdMin = qtdMin}
    }
End If
...
Return New With {
    ...
    .codDesc = "Sucesso",
    .message = "",
    .data = New With {.atingiuMinimo = True, .abaixoMinimo = False, .qtdCot = qtdCot, .qtdMin = qtdMin}
}

Catch ex As Exception
    Xcutil.LogViewSource(mpage, ex.ToString())
    Return New With {.cod = "0007", .codDesc = "Erro interno", .message = "Erro interno ao validar mínimo de cotações.", .data = Nothing}
End Try
